...
cdc_flu <- filter(flu_file, grepl("IVY", received_source))
...
cdc_flu <- cdc_flu %>% select(sample_id, subject_id, coll_date, flag, received_source, 
                              ...
                              nextclade_HA_type, Isolate_Id, PB2.Segment_ID, PB1.Segment_ID, 
                              PA.Segment_ID, HA.Segment_ID, NP.Segment_ID, NA.Segment_ID, 
                              MP.Segment_ID, NS.Segment_ID, Isolate_Name)
...
colnames(cdc_flu) <- c("sample_id", "study_id", "coll_date_flu", "flag_flu", 
                       ...
                       "nextclade_ha_totalnonacgtns_flu", "nextclade_ha_type_flu", "pb2_segment_id_flu", 
...
write.csv(cdc_flu, paste0("/Users/juliegil/Dropbox (University of Michigan)/MED-LauringLab/", 
                          "SEQUENCING/INFLUENZA_A/4_SequenceSampleMetadata/FinalSummary/", 
                          "IVY_uploads/cdc_ivy_flu_", today_date, ".csv"))

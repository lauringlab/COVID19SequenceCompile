...
outputLOC <- paste0(starting_path, "SEQUENCING/hMPV/3_ProcessedGenomes/", plate_name, "/")

# read in compiled dataset
seq_list <- read.csv(paste0(starting_path, "SEQUENCING/hMPV/4_SequenceSampleMetadata/FinalSummary/full_compiled_data.csv"), colClasses = "character")
...
if (missing_subject_id > (96 - num_rsvs_on_plate)){
  print("Check manifests for:")
  print(plate_name)
  stop("Subject IDs Missing")
}

if (missing_collection_date > (96 - num_rsvs_on_plate)){
  print("Check manifests for:")
  print(plate_name)
  stop("Collection Dates Missing")
}

if (missing_received_source > (96 - num_rsvs_on_plate)){
  print("Check manifests for:")
  print(plate_name)
